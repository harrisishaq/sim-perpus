...
class LaporanPengembalianController extends Controller
{
    ...
    public function show()
    {
     ...
     //    	->get();
        // dd($transaksi);
    	return view('report.pengembalian.index', ['data' => $transaksi]);
    }
    ...
    public function report(Request $request)
    {
        ...
        if($choice == "all"){
        	...
        	$pdf = PDF::loadview('report.pengembalian.export-all-pdf',[
                'report'=>$data,
                ])->setPaper([0,0, 612, 935.433], 'landscape');
            return $pdf->stream('All-Returned-Transaction.pdf');
        ...
        elseif($choice == "nim")
        {
            ...
            {
            	return redirect('laporan/pengembalian')->withErrors(__('Gagal mengunduh laporan, pastikan anda telah mengisi NIM dari Mahasiswa yang diinginkan.'));
            } else {
            ...
            $pdf = PDF::loadview('report.pengembalian.export-mahasiswa-pdf',[
                'report'=>$data,
        ...
        elseif($choice == "month")
        {
        	...
        	$data = Transaksi::where('date_returned','>=', $first_day)->where('date_returned', '<=', $last_day)->orderBy('date_returned', 'asc')->get();
                ...
                $pdf = PDF::loadview('report.pengembalian.export-date-pdf',[
                'report'=>$data,
                'date_from'=>$first_day,
                'date_until'=>$last_day,
                ])->setPaper([0,0, 612, 935.433], 'landscape');
        ...
        elseif($choice == "specific")
        {
            ...
            if($date_from != null || $date_until != null)
            {
            	...
            	return redirect('laporan/pengembalian')->withErrors(__('Gagal mengunduh laporan, pastikan anda telah mengisi rentang tanggal yang diinginkan dengan benar.'));
            }
        }
    }
}
